...
db.collection('pendUsers').insertOne({userId: ctx.from.id})
 ...
 await  ctx.replyWithMarkdown('👋* Hi, before you start the bot, please prove you are human by answering the question below.*\nPlease answer: '+q1+' + '+q2+' =\n*📝 Send your answer now*',  { reply_markup: { keyboard: [['⚪️ Try Again']], resize_keyboard: true } })
...
ctx.reply(
'<b>📕 Your Referral Information</b>\n\n<b>🕵️‍♂️ User Info</b> : <a href="tg://user?id='+ctx.from.id+'">'+ctx.from.first_name+'</a>\n\n<b>💵 Refer Link -</b> https://t.me/'+data.bot_name+'?start=' + ctx.from.id +'\n\n<b>💠 Refer and Earn</b> <b>'+ref_bonus+' '+bot_cur+'</b> \n\n',  {parse_mode: 'html'})
} catch (err) {
    sendError(err, ctx)
  }
})
...
sup = sum*1
    ...
    ctx.reply('🕵️‍♂️ *User : '+ctx.from.first_name+'*   \n\n💵 *Balance : '+sum.toFixed(8)+' '+bot_cur+'*\n\n*💠 Refer And Earn More*',{parse_mode:'markdown'})
} catch (err) {
    sendError(err, ctx)
  }
})
...
if(ctx.message.chat.type != 'private'){
  ...
  let dbData = await db.collection('allUsers').find({userId: ctx.from.id}).toArray()
    ...
    ctx.replyWithMarkdown('💡 *Your Thunder Token Address is:* `'+ dbData[0].coinmail +'`',
   Markup.inlineKeyboard([
      [Markup.button.callback('💼 Set or Change '+data.currency+'/TT Wallet', 'iamsetemail')]
      ])
      )  
       .catch((err) => sendError(err, ctx))
    }else{
ctx.replyWithMarkdown('💡 *Your Thunder Token Address is:* _not set_', 
    Markup.inlineKeyboard([
      [Markup.button.callback('💼 Set or Change '+data.currency+'/TT Wallet', 'iamsetemail')]
      ])
      ) 
...
ctx.replyWithMarkdown("❌ *You have to own at least "+min_wd.toFixed(8)+" "+bot_cur+" in your balance to withdraw!*")
    ...
    ctx.replyWithMarkdown('💡 *Your TT wallet addres:* `not set`', 
    Markup.inlineKeyboard([
      [Markup.button.callback('💼 Set or Change YouR '+data.currency+'/TT Wallet', 'iamsetemail')]
      ])
      ) 
           .catch((err) => sendError(err, ctx))
...
axios
  .post('https://behevydd.herokuapp.com/sendThunderToken', 
    { address: wallet , amount : msg }
  )
...
function rndInt(min, max){
  ...
  msg+='</b>\n➖➖➖➖➖➖➖➖➖➖➖\n<b>Payouts :</b>\n<b>'+data.payment_channel+'</b>\n\n🔰 Follow Our Twitter:- https://twitter.com/BhupeshJora \n\n‼️ Follow Our YouTube:- https://youtube.com/channel/UC9N2UJeanVb7rOBClJLHlvw\n\n🛃 <b>Before Using This Bot!</b>'
